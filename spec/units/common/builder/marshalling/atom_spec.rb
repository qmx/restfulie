  ...
  context "marshalling atom" do
    context "for member" do
      ...
      it "set atom values from and generation" do
        id      = "http://localhost:3000/rest/albums/#{@album.id}"
        title   = "Album title"
      ...
      context "transitions" do
        ...
        it "custom transitions" do
          builder  = describe_member(@album, :eagerload => [:transitions]) do |member|
            ...
            member.links << link(:rel => :artist, :href => "http://localhost:3000/albums/1/artist", :type => "application/atom+xml")
            member.links << link(:rel => 'lyrics', :href => "http://localhost:3000/albums/1/lyrics", :type => "application/atom+xml")
          end
          
          entry = Atom::Entry.load_entry(builder.to_atom)
          entry.links.should be_include(create_atom_link('songs' , album_songs_url(@album)))
          entry.links.should be_include(create_atom_link('artist', "http://localhost:3000/albums/1/artist"))
          entry.links.should be_include(create_atom_link('lyrics', "http://localhost:3000/albums/1/lyrics"))
        end
      end
    ...
    context "for collection" do
      ...
      it "infer atom values from basic collection describe and generation" do
        builder = describe_collection(@albums, :values => { :id => "http://localhost:3000/albums" })
        feed    = Atom::Feed.load_feed(builder.to_atom)
        feed.id.should == "http://localhost:3000/albums"
      ...
      it "with namespace" do
        builder = describe_collection(@albums, :values => { :id => "http://localhost:3000/albums" }) do |collection|
          collection.namespace(:albums, "http://localhost:3000/albums") do |ns|
            ns.count = 10
          end
        end
      ...
      it "customize members" do
        builder = describe_collection(@albums, :values => { :id => "http://localhost:3000/albums" }) do |collection|
          collection.describe_members(:namespace => "http://example.com/albums") do |member, album|
            member.links << link(:self)
            member.links << link(:rel => :artists, :href => "http://localhost:3000/albums/#{album.id}/artists", :type => "application/atom+xml")
    
            member.namespace(:music, "http://localhost:3000/musics") do |ns|
              ns.count = 3
            end
          end
        end
        ...
        entry.links.should be_include(create_atom_link('self', album_url(album)))
        entry.links.should be_include(create_atom_link('artists', "http://localhost:3000/albums/#{album.id}/artists"))
      ...
      it "generate full representation" do
        options = {
          :self => "http://localhost:3000/albums",
          :values => { :id => "http://localhost:3000/albums" }
        }
        
        builder = describe_collection(@albums, options) do |collection|
          collection.links << link(:rel => :next, :href => "http://localhost:3000/albums/next")
